...
export default async function handler(req: VercelRequest, res: VercelResponse): Promise<void> {
  ...
  const commands = dateKeys.map((dateKey) => ["HGETALL", `clicks:source:${dateKey}`]);
  try {
    const responses = await upstash.pipeline(commands);
    const rows: Array<{ date: string; source_id: string; clicks: number }> = [];
    ...
    responses.forEach((item, index) => {
      ...
      for (const [sourceId, clicks] of Object.entries(clicksBySource)) {
        rows.push({
          date,
          source_id: sourceId,
          clicks,

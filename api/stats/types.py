...
class handler(BaseHTTPRequestHandler):
    def do_GET(self) -> None:  # noqa: N802
        ...
        commands = [["HGETALL", f"clicks:type:{date_key}"] for date_key in date_keys]
        ...
        for index, item in enumerate(responses):
            ...
            clicks_by_type = parse_hash_result(payload)
            date = key_to_iso_date(date_keys[index])
            for primary_type, clicks in clicks_by_type.items():
                rows.append(
                    {
                        "date": date,
        ...
        rows.sort(key=lambda row: (str(row["date"]), str(row["primary_type"])))
        _respond_json(
            self,
            200,
            {
